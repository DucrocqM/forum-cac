 ...
 *              Allows the model to return the resusiteslts as object or as array
 ...
 *          $this->cache_driver = 'file'sites
 *          $this->cache_prefix = 'mm'
 *              If you know you will do sitesg of results without the native caching solution, you can at any time use the MY_Model's caching.
 *              By default, MY_Model usesites to cache result.
...
class MY_Model extends CI_Model
{
    ...
    public function insert($data = null)
    {
        ...
        if ($multi === false) {
            ...
            if ($this->_database->insert($this->table, $data)) {
                ...
                if ($return === false) {
                    $_SESSION['warning'][] = "Impossible d'effectuer une insertion sur la table '$this->table' ";
                } else {
                    $_SESSION['success'][] = "Insertion sur la table '$this->table ' ";
                }
                return $return;
            }
            return false;
        ...
        if ($multi === false) {
            ...
            if ($escape) {
                if ($this->_database->update($this->table, $data)) {
                    ...
                    if ($return === false) {
                        $_SESSION['warning'][] = "Impossible d'effectuer une maj sur la table '$this->table' ";
                    } else {
                        $_SESSION['success'][] = "Maj sur la table '$this->table ' ";
                    }
                    return $return;
                }
            } else {
    ...
    public function delete($where = null)
    {
        ...
        if ($this->soft_deletes === true) {
            ...
            if ($affected_rows == 0) {
                $_SESSION['warning'][] = "Impossible d'effectuer un soft delete sur la table '$this->table' ";
            } else {
                $_SESSION['success'][] = "Soft delete sur la table '$this->table ' ";
            }
            ...
            if ($this->_database->delete($this->table)) {
                ...
                $this->_prep_after_write();
                if ($affected_rows == 0) {
                    $_SESSION['warning'][] = "Impossible d'effectuer un hard delete sur la table '$this->table' ";
                } else {
                    $_SESSION['success'][] = "Hard delete sur la table '$this->table ' ";
                }
                return $affected_rows;
            }
        }
    ...
    public function force_delete($where = null)
    {
        ...
        if ($this->_database->delete($this->table)) {
            $this->_prep_after_write();
            if ($affected_rows == 0) {
                $_SESSION['warning'][] = "Impossible d'effectuer un force delete sur la table '$this->table' ";
            } else {
                $_SESSION['success'][] = "Force delete sur la table '$this->table ' ";
            }
            if ($this->_database->affected_rows() === false) {
                $_SESSION['warning'][] = "Impossible d'effectuer un force delete sur la table '$this->table' ";
            } else {
                $_SESSION['success'][] = "Force delete sur la table '$this->table ' ";
            }
    ...
    public function restore($where = null)
    {
        ...
        if ($affected_rows === false) {
            $_SESSION['warning'][] = "Impossible d'effectuer une restauration sur la table '$this->table' ";
        } else {
            $_SESSION['success'][] = "Restauration sur la table '$this->table ' ";
        }
        return false;
    }
    ...
    public function trashed($where = null)
    {
        ...
        if ($query->num_rows() == 1) {
            return true;
            $_SESSION['success'][] = "Mise à la corbeille sur la table '$this->table ' ";
        }
        $_SESSION['warning'][] = "Impossible d'effectuer une mise à la corbeille sur la table '$this->table' ";
        return false;
    }
    ...
    {
        $next = $this->db->query("SHOW TABLE STATUS LIKE '" . $this->table . "'");
        $next = $next->row(0)->Auto_increment;
        return   $next;
        //return $this->db->query('SELECT MAX(id) AS `maxid` FROM `' . $this->table . '`')->row()->maxid;
        //return array_reverse($this->as_array()->get_all())[0]['id'];
    }
    /**
